 ...
 * $ mpicc.mpich 'comp_MPIpy copy.c' -shared -fPIC -o comp_MPIpy.so
...
void mpi_gather(const void* sendbuf, int sendcount, int datatypeKey, void* recvbuf, int recvcount, int root, int comm);
...
void mpi_gather(const void* sendbuf, int sendcount, int datatypeKey, void* recvbuf, int recvcount, int root, int comm)
{
	...
	MPI_Gather(sendbuf, sendcount, data, recvbuf, recvcount, data, root, comm);
}
